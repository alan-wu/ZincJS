...
// reference implementation, for repeat_mode "AXES_2D", using a real WebGPU
...
describe('glyph transform compute (AXES_2D)', () => {
  it('matches the CPU resolve_glyph_axes reference for a blended frame', async () => {
    await runRepeatModeCheck(webgpuRenderer, "AXES_2D", createGlyphTransformCompute, dispatchAndReadbackGlyphTransform, expect);
  });
});
