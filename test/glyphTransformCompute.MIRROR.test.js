...
// reference implementation, for repeat_mode "MIRROR", using a real WebGPU
...
describe('glyph transform compute (MIRROR)', () => {
  it('matches the CPU resolve_glyph_axes reference for a blended frame', async () => {
    await runRepeatModeCheck(webgpuRenderer, "MIRROR", createGlyphTransformCompute, dispatchAndReadbackGlyphTransform, expect);
  });
});
